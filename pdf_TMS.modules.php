...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_TMS.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from TMS model
 */
...
/**
 *	Class to manage PDF invoice template TMS
 */
class pdf_TMS extends ModelePDFFactures
{
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "TMS";
		$this->description = "Facturacion Argentina por TMS Group S.A.";
			...
			if (file_exists($dir))
			{
				...
				*/
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					$pdf->MultiCell(10, 3, $qty, 0, 'R');	// Enough for 6 chars
					...
					$pdf->MultiCell(13, 3, $up_excl_tax, 0, 'R', 0);
	...
	//		$pdf->line($this->posxtva-1, $tab_top, $this->posxtva-1, $tab_top + $tab_height);
			if (empty($hidetop))
			{
				$pdf->SetXY($this->posxtva-13, $tab_top+1);
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$logo=$conf->mycompany->dir_output.'/logos/facturaa.jpg';
		if ($this->emetteur->logo)
		{
			if (is_readable($logo))
			{
				...
				$pdf->Image($logo, 0, 0, 0, 400);	// width=0 (auto)
			}
			else
			{
				$pdf->SetTextColor(200,0,0);
		...
		$pdf->MultiCell(100, 3, dol_print_date($object->date,"day",false,$outputlangs), '', 'R');
	...
	*/
		...
		*/
		...
		if ($object->client->typent_code == 'A_IVARE')
		{
			$posy+=3;
			$pdf->SetFont('','B', $default_font_size + 2);
                        $pdf->SetXY($posx-15,$posy+47);
                        $pdf->MultiCell(100, 3, "Responsable Inscripto", '', 'R');
		}
		if ($object->client->typent_code == 'B_EXENTO')
		{
			$posy+=3;
			$pdf->SetFont('','B', $default_font_size + 2);
                        $pdf->SetXY($posx-15,$posy+37);
                        $pdf->MultiCell(100, 3, "Exento", '', 'R');
		}
		if ($object->client->typent_code == 'B_FINAL')
		{
			$posy+=3;
			$pdf->SetFont('','B', $default_font_size + 2);
                        $pdf->SetXY($posx-15,$posy+37);
                        $pdf->MultiCell(100, 3, "Consumidor Final", '', 'R');
		}
		if ($object->client->typent_code == 'C_MONOTRIB')
		{
			$posy+=3;
			$pdf->SetFont('','B', $default_font_size + 2);
                        $pdf->SetXY($posx-15,$posy+37);
                        $pdf->MultiCell(100, 3, "Monotributista", '', 'R');
		}
		$posy+=1;
		...
		if ($showaddress)
		{
			...
			*/
			...
			// Nombre Cliente
			$pdf->SetXY($posx-70,$posy+28);
			$pdf->SetFont('','B', $default_font_size);
			$pdf->MultiCell($widthrecbox, 4, $carac_client_name, 0, 'L');

			// Show recipient information
			$pdf->SetFont('','', $default_font_size - 1);
			$pdf->SetXY($posx-70,$posy+36+(dol_nboflines_bis($carac_client_name,50)*4));
